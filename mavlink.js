...
mavlink.MAV_CMD_VIDEO_START_CAPTURE = 2500 // Starts video capture (recording)
mavlink.MAV_CMD_VIDEO_STOP_CAPTURE = 2501 // Stop the current video capture (recording)
...
mavlink.MAV_CMD_NAV_FENCE_POLYGON_VERTEX_INCLUSION = 5001 // Fence vertex for an inclusion polygon. The vehicle must stay within
                        // this area. Minimum of 3 vertices required.
mavlink.MAV_CMD_NAV_FENCE_POLYGON_VERTEX_EXCLUSION = 5002 // Fence vertex for an exclusion polygon. The vehicle must stay outside
                        // this area. Minimum of 3 vertices required.
...
mavlink.MAV_PROTOCOL_CAPABILITY_ENUM_END = 32769 // 
...
mavlink.MAVLINK_MSG_ID_BEACON_CONFIG = 227
mavlink.MAVLINK_MSG_ID_BEACON_DIS = 228
mavlink.MAVLINK_MSG_ID_BEACON_POS = 229
...
/* 
                ...
                payload                   : payload size per packet (normally 254 byte, see DATA field size in message ENCAPSULATED_DATA) (set on ACK only) (uint8_t)
...
Beacon Location
                ...
                beacon_id                 : ID (uint8_t)
                beacon_x                  : X (int32_t)
                beacon_y                  : Y (int32_t)
                beacon_z                  : Z (int32_t)

*/
mavlink.messages.beacon_config = function(beacon_id, beacon_x, beacon_y, beacon_z) {

    this.format = '<iiiB';
    this.id = mavlink.MAVLINK_MSG_ID_BEACON_CONFIG;
    this.order_map = [3, 0, 1, 2];
    this.crc_extra = 23;
    this.name = 'BEACON_CONFIG';

    this.fieldnames = ['beacon_id', 'beacon_x', 'beacon_y', 'beacon_z'];
...
mavlink.messages.beacon_config.prototype = new mavlink.message;

mavlink.messages.beacon_config.prototype.pack = function(mav) {
    return mavlink.message.prototype.pack.call(this, mav, this.crc_extra, jspack.Pack(this.format, [ this.beacon_x, this.beacon_y, this.beacon_z, this.beacon_id]));
}
...
Beacon Distance
                ...
                beacon_id                 : ID (uint8_t)
                beacon_m                  : distance in meters (int32_t)

*/
mavlink.messages.beacon_dis = function(beacon_id, beacon_m) {

    this.format = '<iB';
    this.id = mavlink.MAVLINK_MSG_ID_BEACON_DIS;
    this.order_map = [1, 0];
    this.crc_extra = 83;
    this.name = 'BEACON_DIS';

    this.fieldnames = ['beacon_id', 'beacon_m'];
...
mavlink.messages.beacon_dis.prototype = new mavlink.message;

mavlink.messages.beacon_dis.prototype.pack = function(mav) {
    return mavlink.message.prototype.pack.call(this, mav, this.crc_extra, jspack.Pack(this.format, [ this.beacon_m, this.beacon_id]));
}
...
Vehicle Position
                ...
                vehicle_y                 : Y (int32_t)
                vehicle_z                 : Z (int32_t)
                vehicle_err               : position error (int16_t)

*/
mavlink.messages.beacon_pos = function(vehicle_x, vehicle_y, vehicle_z, vehicle_err) {

    this.format = '<iiih';
    this.id = mavlink.MAVLINK_MSG_ID_BEACON_POS;
    this.order_map = [0, 1, 2, 3];
    this.crc_extra = 61;
    this.name = 'BEACON_POS';

    this.fieldnames = ['vehicle_x', 'vehicle_y', 'vehicle_z', 'vehicle_err'];
...
mavlink.messages.beacon_pos.prototype = new mavlink.message;

mavlink.messages.beacon_pos.prototype.pack = function(mav) {
    return mavlink.message.prototype.pack.call(this, mav, this.crc_extra, jspack.Pack(this.format, [ this.vehicle_x, this.vehicle_y, this.vehicle_z, this.vehicle_err]));
}
...
*/
mavlink.messages.camera_capture_status = function(time_boot_ms, camera_id, image_status, video_status, image_interval, video_framerate, image_resolution_h, image_resolution_v, video_resolution_h, video_resolution_v, recording_time_ms, available_capacity) {

    this.format = '<IffIfHHHHBBB';
    this.id = mavlink.MAVLINK_MSG_ID_CAMERA_CAPTURE_STATUS;
    this.order_map = [0, 9, 10, 11, 1, 2, 5, 6, 7, 8, 3, 4];
    this.crc_extra = 69;
    this.name = 'CAMERA_CAPTURE_STATUS';

    this.fieldnames = ['time_boot_ms', 'camera_id', 'image_status', 'video_status', 'image_interval', 'video_framerate', 'image_resolution_h', 'image_resolution_v', 'video_resolution_h', 'video_resolution_v', 'recording_time_ms', 'available_capacity'];
...
mavlink.messages.camera_capture_status.prototype.pack = function(mav) {
    return mavlink.message.prototype.pack.call(this, mav, this.crc_extra, jspack.Pack(this.format, [ this.time_boot_ms, this.image_interval, this.video_framerate, this.recording_time_ms, this.available_capacity, this.image_resolution_h, this.image_resolution_v, this.video_resolution_h, this.video_resolution_v, this.camera_id, this.image_status, this.video_status]));
}
...
mavlink.map = {
        ...
        131: { format: '<H254s', type: mavlink.messages.encapsulated_data, order_map: [0, 1], crc_extra: 223 },
        ...
        227: { format: '<iiiB', type: mavlink.messages.beacon_config, order_map: [3, 0, 1, 2], crc_extra: 23 },
        228: { format: '<iB', type: mavlink.messages.beacon_dis, order_map: [1, 0], crc_extra: 83 },
        229: { format: '<iiih', type: mavlink.messages.beacon_pos, order_map: [0, 1, 2, 3], crc_extra: 61 },
        ...
        254: { format: '<B50s', type: mavlink.messages.statustext, order_map: [0, 1], crc_extra: 83 },
        ...
        262: { format: '<IffIfHHHHBBB', type: mavlink.messages.camera_capture_status, order_map: [0, 9, 10, 11, 1, 2, 5, 6, 7, 8, 3, 4], crc_extra: 69 },
